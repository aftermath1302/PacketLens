...
std::string GlobalConnectionTable::generateReport() const {
    ...
    out << "Active Connections      : " << stats.total_active_connections << "\n";
    out << "Total Connections Seen  : " << stats.total_connections_seen << "\n\n";

    //Application Breakdown
    out << "---------------- Application Breakdown ------------------\n";

    // Calculate total connections for percentage calculation
    size_t total = 0;
    for (const auto& entry : stats.app_distribution) {
        total += entry.second;
    }

    // Move to vector and sort by count (descending)
    std::vector<std::pair<AppType, size_t>> apps(
    ...
    for (const auto& entry : apps) {
        ...
        if (total > 0) {
            percentage = (100.0 * entry.second) / total;
        }

        out << std::left << std::setw(20) << appTypeToString(entry.first)
    ...
    if (!stats.top_domains.empty()) {
        ...
        for (const auto& entry : stats.top_domains) {
            ...
            if (domain.length() > 40) {
                domain = domain.substr(0, 37) + "...";
            }

            out << std::left << std::setw(45) << domain
                << std::right << entry.second << "\n";
        }
    }
