...
Architecture
  ...
  1. PCAP Reader
     Reads packets from the input file and extracts the 5-tuple.

  2. Load Balancer
     ------------------------
     hash(5-tuple) % num_lbs
     ------------------------
     Distributes flows across load balancer threads.

  3. Fast Path Processors
     ---------------------------
     hash(5-tuple) % fps_per_lb
     ---------------------------
     Each LB distributes flows across FP workers.

     Workers perform DPI, application classification,
     and rule evaluation.

  4. Output Writer
     Writes forwarded packets to the output PCAP file.
