...
std::string FPManager::generateClassificationReport() const {
    ...
    std::ostringstream ss;
        ...
        double classified_pct = total ? (100.0 * total_classified / total) : 0.0;
        double unknown_pct = total ? (100.0 * total_unknown / total) : 0.0;

        ss << "Total Connections : " << total << "\n";
        ss << "Classified        : " << total_classified
        << " (" << std::fixed << std::setprecision(1)
        << classified_pct << "%)\n";

        ss << "Unidentified      : " << total_unknown
        << " (" << unknown_pct << "%)\n\n";
